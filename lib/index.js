...
var SQSProcessor = function (options) {
    ...
    this.createQueue = Promise.promisify (this._queue.createQueue, this._queue)
...
SQSProcessor.prototype.postToWorker = function (messageBody, sqsMessage) {
    ...
    return httpPOST({
        ...
        rejectUnauthorized: (this.options.workerSslEnabled == 'true'),
...
SQSProcessor.prototype.doCheckWorkerHealth = function (beginTimeStamp) {
    ...
    logger.info("try ping worker by " + this.options.workerHealthUrl)
    return httpGET({
        url: this.options.workerHealthUrl,
        rejectUnauthorized: (this.options.workerSslEnabled == 'true')
    })
        .then(function () {
            logger.info("Worker is healthy.")
            return true;
        })
        .catch(e=> {
    ...
    return this.receiveMessage({
        ...
        .then(data => {
            if (!data || !Array.isArray(data.Messages) || data.Messages.length == 0) {
                logger.info("No Messages Received via poll time")
                return;
            }
